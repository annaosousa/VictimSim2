...
class Rescuer(AbstAgent):
    def __init__(self, env, config_file):
        ...
        self.map = {}             # explorer will pass the map
    ...
    def go_save_victims(self, map, victims):
        ...
        # #     x, y = coord
        # #     print(f"{self.NAME} Victim seq number: {
        # #           seq} at ({x}, {y}) vs: {vital_signals}")
        ...
        #     self.plan_x += a[0]
        #     self.plan_y += a[1]
        #     print(f"{self.NAME} {i}) dxy=({a[0]}, {
        #           a[1]}) vic: a[2] => at({self.plan_x}, {self.plan_y})")
        #     i += 1
        ...
        # Atualize self.map e self.victims corretamente
        self.receive_map(map)
        self.known_victims = victims
    ...
    def __depth_search(self, actions_res):
        ...
        for i, ar in enumerate(actions_res):
            ...
            print(f"{self.NAME}: planning to go to ({
                  self.plan_x}, {self.plan_y})")
            ...
            print(f"{self.NAME}: difficulty {
                  difficulty}, step cost {step_cost}")
            print(f"{self.NAME}: accumulated walk time {
                  self.plan_walk_time}, rtime {self.plan_rtime}")

            # check if there is enough remaining time to walk back to the base
            if self.plan_walk_time + step_cost > self.plan_rtime:
                enough_time = False
                print(f"{self.NAME}: no enough time to go to ({
                      self.plan_x}, {self.plan_y})")
            ...
            if enough_time:
                ...
                if vic_seq == VS.NO_VICTIM:
                    ...
                    print(f"{self.NAME}: added to the plan, walk to ({
                          self.plan_x}, {self.plan_y}, False)")
                ...
                if vic_seq != VS.NO_VICTIM:
                    ...
                    if self.plan_rtime - self.COST_FIRST_AID < self.plan_walk_time:
                        ...
                        print(f"{self.NAME}:added to the plan, walk to and rescue victim({
                              self.plan_x}, {self.plan_y}, True)")
    ...
    def __planner(self):
        ...
        # difficulty, vic_seq, actions_res = self.know_map.get((0, 0))
